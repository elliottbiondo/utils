...
double pmt_x[253] = {0, 92, 46, -46, -92, -46, 46, 138, 0, -138, -138, 0, 138,
...
609.467223, 657.407722, 694.099786, 718.915611, 731.430591};

double pmt_y[253] = {0, 0, -79.674337, -79.674337, 0, 79.674337, 79.674337,
...
324.197605, 235.615124, 143.001204, 47.940493};
}
...
G4VPhysicalVolume* SimpleLZ::Construct()
{
    ...
    else
    {
        ...
        double pmt_radius = (pmt_pitch - 1)/2;
        ...
        for (auto i = 0; i < sqrt_num_pmts_; ++i)
        {
            ...
            {
                G4Tubs* pmt = new G4Tubs("pmt", 0, pmt_radius, 123*mm, 0*deg, 360*deg);
                std::string pmt_name = "pmt_" + std::to_string(i);
                auto const pmt_lv = new G4LogicalVolume(pmt, pmt_mat, pmt_name);
                new G4PVPlacement(
                    nullptr, G4ThreeVector({pmt_pitch*i - offset, pmt_pitch*j - offset, 0*mm}), pmt_lv, "pmt_pv", world_lv, false, 0, false);
            }
        }
    }
