...
store_step(TTree* steps_ttree, std::vector<std::string>* action_labels)
{
    ...
    step.energy_loss = steps_ttree->GetLeaf("energy_deposition")->GetValue();
...
//---------------------------------------------------------------------------//
/*!
 * Generate a post-processed ROOT file with a `rootdata::Event` structure from
 * a Celeritas ROOT output file generated by `celeritas::RootStepWriter`.
 *
 * Usage:
 * $ root
 * root[0] .x post_process_celeritas("celeritas-mctruth.root")
 */
void post_process_celeritas(std::string input_filename)
{
    ...
    // Open Celeritas RootStepWriter file
    auto input = TFile::Open(input_filename.c_str(), "read");
    auto steps_tree = input->Get<TTree>("steps");
    auto params_tree = input->Get<TTree>("core_params");
    if (!input || !steps_tree || !params_tree)
    {
        std::cout
            << "Input file is not found or is not generated by Celeritas' "
               "RootStepWriter"
            << std::endl;
        return;
    }
    std::cout << "Open " << input_filename << std::endl;

    std::vector<std::string>* action_labels = nullptr;
    params_tree->SetBranchAddress("action_labels", &action_labels);
    params_tree->GetEntry(0);  // Load labels
    ...
    ProgressIndicator progress(steps_tree->GetEntries());
