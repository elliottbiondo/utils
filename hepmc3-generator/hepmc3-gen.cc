                      ...
                      const double       particle_energy)
{
    ...
    for (int event = 0; event < num_events; event++)
    {
        ...
        for (int part_idx = 0; part_idx < num_part_per_event; part_idx++)
        {
            ...
            const double theta = 2 * pi * uniform(rng);
            const double phi   = acos(2 * uniform(rng) - 1.0);
...
//---------------------------------------------------------------------------//
/*!
 * Create a particle-gun output with fixed direction and vertex, and uniform
 * energy range.
 */
void create_particle_gun(const char*        hepmc3_output,
                         const unsigned int num_events,
                         ...
                         const double       vertex[3])
{
    ...
    std::mt19937                           rng(12345);
    std::uniform_real_distribution<double> uniform(energy_min, energy_max);
 ...
 * - Particle gun source with uniform energy range, and fixed direction and
 *   position.
 ...
 * ./hepmc3-gen [particle_gun.hepmc3] [num_events] [num_part_per_evt]
 *              [pdg_id] [mev_energy_min] [mev_energy_max] [cm_direction]
 *              [cm_vertex]
...
int main(int argc, char* argv[])
{
    ...
    else if (argc == 13)
    {
        ...
        const unsigned int num_part_per_event = std::stoul(argv[3]);
        const int          pdg                = std::stoi(argv[4]);
        const double       energy_min         = std::stod(argv[5]);
    ...
    else
    {
        ...
        std::cout << argv[0]
                  << " [particle_gun_out.hepmc3] [num events] "
                     "[num particles per event] [pdg] [particle min energy "
                     "(MeV)] [particle max energy (MeV)] [particle directions "
                     "(x, y, z) (unity)] [particle vertex (x, y, z) (cm)]"
                  << std::endl;
